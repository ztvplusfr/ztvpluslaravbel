...
<body class="bg-background text-text">
    ...
    <main class="pt-20">
        ...
        <div class="relative w-full bg-gray-900 py-12 px-4 sm:px-6 lg:px-8">
            <div class="text-left">
                <h1 class="text-4xl md:text-5xl font-bold text-white mb-4">Séries</h1>
                <p class="text-lg text-gray-300 mb-8 max-w-2xl">Plongez dans nos séries captivantes, des thrillers passionnants aux drames émouvants.</p>
            </div>
        </div>
        ...
        <div class="min-h-screen">
            @if($series && $series->isNotEmpty())
                ...
                <section class="py-8 w-full px-0 sm:px-6 lg:px-0 lg:pl-12 mx-0">
                    <h2 class="text-white text-2xl font-bold mb-6 ml-4 lg:ml-0">Toutes les séries</h2>
                    <div class="overflow-x-auto scrollbar-hide pb-4">
                        <div class="flex space-x-4 px-4 lg:px-0 min-w-max">
                            @foreach($series as $serie)
                                <a href="{{ route('series.show', $serie->id) }}" class="flex-shrink-0 w-48 sm:w-52 md:w-56 relative group block">
                                    @if($serie->poster_path)
                                        <img src="https://image.tmdb.org/t/p/w500{{ $serie->poster_path }}" alt="{{ $serie->title }}" class="w-full h-auto aspect-[2/3] object-cover rounded-lg">
                                    @else
            ...
            @else
                <div class="flex items-center justify-center min-h-96">
                    <div class="text-center">
                        ...
                        <h3 class="text-xl font-medium text-gray-300 mb-2">Aucune série disponible</h3>
                        <p class="text-gray-400">Nous travaillons à ajouter de nouvelles séries à notre collection.</p>
                    </div>
                </div>
            @endif
        </div>
