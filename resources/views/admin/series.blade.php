...
<head>
     ...
     <title>Administration - Séries</title>
 ...
 <body class="bg-background text-white">
     <div class="min-h-screen flex">
         ...
         <main class="flex-1 p-8 h-screen overflow-y-auto">
             <h1 class="text-3xl font-bold text-primary mb-6">Gestion des Séries</h1>
             <p class="mb-4">Bienvenue, {{ Auth::user()->name }} ! Voici la liste des séries.</p>

             <!-- Liste des séries en tableau -->
                     ...
                     <tbody class="divide-y divide-primary">
                         ...
                         <tr class="bg-background hover:bg-primary hover:bg-opacity-10">
                             ...
                             <td class="px-4 py-4 whitespace-nowrap text-center">
                                 ...
                                 <img src="https://image.tmdb.org/t/p/w92{{ $serie->poster_path }}" alt="{{ $serie->title }}" class="w-12 h-16 object-cover">
                                 @else
                                 <div class="w-12 h-16 bg-background flex items-center justify-center text-xs text-text">N/A</div>
                                 @endif
                             </td>
                             <td class="px-4 py-4 whitespace-nowrap text-center text-sm text-text">{{ $serie->title }}</td>
                             ...
                             <td class="px-4 py-4 whitespace-nowrap text-center text-sm text-text">{{ $serie->first_air_date ? $serie->first_air_date->format('Y') : 'N/A' }}</td>
                             <td class="px-4 py-4 whitespace-nowrap text-center text-sm text-text">{{ $serie->genres ? implode(', ', $serie->genres) : 'N/A' }}</td>
                             <td class="px-4 py-4 whitespace-nowrap text-center text-sm">
                                 <div class="flex justify-center space-x-2">
                                     <a href="{{ route('admin.series.edit', $serie->id) }}" class="bg-primary text-background px-3 py-1 rounded text-xs font-medium hover:bg-opacity-80">Modifier</a>
             ...
             @if($series->isEmpty())
             <p class="text-gray-400 mt-4">Aucune série trouvée.</p>
             @endif
         </main>
     </div>
 </body>
